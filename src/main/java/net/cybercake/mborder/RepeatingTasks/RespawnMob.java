...
public class RespawnMob implements Runnable{
    ...
    public void run() {
        if(DataUtils.getCustomYmlBoolean("data", "server.active")) {
            ...
            if(diff >= Main.getMainConfig().getLong("mobRespawnInterval")) {
    ...
    public static void respawnEntity(String worldType) {
        ...
        Location location = Utils.getTopBlock(DataUtils.getCustomYmlLocation("data", "server." + worldType + ".centerLocation"), 100);
